...
class TouchImageEventManager: ITouchImageEventManager
{
    ...
    private ResultReturnType getTouchResult(System.Numerics.Vector2 touchedCoordinates, bool isUpSideImageClicked)
    {
        ...
        {
            bool isInsideHorizontal = ans.X >= touchedCoordinates.X - Constants.correctTouchRange && ans.X <= touchedCoordinates.X + Constants.correctTouchRange;
            bool isInsideVertical = ans.Y >= touchedCoordinates.Y - Constants.correctTouchRange && ans.Y <= touchedCoordinates.Y + Constants.correctTouchRange;
            if (isInsideHorizontal && isInsideVertical) 
            {
